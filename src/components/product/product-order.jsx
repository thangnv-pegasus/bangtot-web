...
const ProductOrder = ({ product = {}, quantity = 1, size = {} }) => {
  ...
  return (
    <div className="p-2 flex justify-between">
      <div className="flex select-none pr-5">
        <div className="relative w-auto h-20 block border-[1px] border-solid border-gray-300 rounded-md">
          <img
            src={image[0]?.name}
        ...
        <Link to={`/chi-tiet-san-pham/${product.id}`} className="ml-5 block w-auto text-sm">
          {product.name}{" "}
          <span className="opacity-80">({product.size_name || size.name})</span>
        </Link>
      </div>
      <div className="text-base flex-1 font-medium text-baseColor">
